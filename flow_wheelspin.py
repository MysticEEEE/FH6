"""自动抽奖流程（游戏内老虎机抽奖 / 超级抽奖）。

上游模块风格：所有函数取 `self`（FH_UltimateBot 实例）为首参，由 main.py 里的
薄 wrapper 绑定调用；类持有全部状态与识图/按键原语。

与上游 flow_cj.logic_super_wheelspin（技能点/车辆精通刷取）不是同一功能——
本模块是真正的游戏内转盘抽奖：连续抽奖、抽到已拥有重复车自动出售、跳过动画，
直到退回「我的地平线」菜单或达到次数上限。纯停止判断见 wheelspin_logic.py。
"""
...
def wheelspin_advance_to_result(self, budget=22.0):
    ...
    while time.time() < deadline:
        if not self.is_running:
            return False
        self.check_pause()
        prompt = _wheelspin_prompt_region(self)
        # 1. 结果界面就绪 → 立刻返回
        ...
        if time.time() - last_menu_check > 1.0:
            ...
            if is_wheelspin_finished(self):
                return False
        time.sleep(0.12)
    return self.find_image_gray("wheelspin/respin.png", region=_wheelspin_prompt_region(self),
...
def collect_and_respin(self):
    ...
    if pos is None:
        pos = self.find_image_gray("wheelspin/respin.png", region=_wheelspin_prompt_region(self),
...
def logic_auto_wheelspin(self):
    ...
    if not navigate_to_wheelspin(self):
        return False
    ...
    while self.is_running:
        ...
        ready = wheelspin_advance_to_result(self)
        if not ready:
